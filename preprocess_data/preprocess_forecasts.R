    ...
    for (as_of in as.character(as_ofs)) {
        ...
        for (location in unique(all_forecasts$location)) {
            target_filename <- paste0(
                "./Covid-19-Hub-Vizualization/static/data/forecasts/",
...
for (target_var in c("case", "death", "hosp")) {
    ...
    for (as_of in as.character(as_ofs)) {
        for (location in locations) {
            file_path <- paste0("./Covid-19-Hub-Vizualization/static/data/forecasts/",
                target_var, "_",
                location, "_",
                as_of,  ".json")
...
writeLines(jsonlite::toJSON(models), "./Covid-19-Hub-Vizualization/static/data/models.json")
